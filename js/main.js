...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateLanguage() {
    ...
    document.title =
      currentLanguage === "zh"
        ? "名偵探柯南犯罪手法大全"
        : currentLanguage === "en"
        ? "Detective Conan Crime Methods Encyclopedia"
        : currentLanguage === "ja"
        ? "名探偵コナン犯罪手法大全"
        : "명탐정 코난 범죄 수법 대전";
    ...
    menuLinks.forEach((link) => {
      ...
      if (menuItems[section]) {
        link.innerHTML = `<i class="${link.querySelector("i").className}"></i>${
          menuItems[section][currentLanguage]
        }`;
      }
    });
    ...
    if (heroTitle) {
      heroTitle.textContent =
        currentLanguage === "zh"
          ? "探索名偵探柯南的犯罪手法世界"
          : currentLanguage === "en"
          ? "Explore the World of Detective Conan Crime Methods"
          : currentLanguage === "ja"
          ? "名探偵コナンの犯罪手法の世界を探る"
          : "명탐정 코난의 범죄 수법 세계 탐험";
    }
    ...
    if (heroDescription) {
      heroDescription.textContent =
        currentLanguage === "zh"
          ? "深入分析柯南系列中的各種犯罪手法，從密室殺人到不在場證明，從心理誘導到科技犯罪，全面解析推理世界的精彩奧秘。"
          : currentLanguage === "en"
          ? "In-depth analysis of various crime methods in the Conan series, from locked room murders to alibis, from psychological manipulation to tech crimes, comprehensively decoding the fascinating mysteries of the detective world."
          : currentLanguage === "ja"
          ? "コナンシリーズに登場する様々な犯罪手法を深く分析します。密室殺人からアリバイ、心理的操作からハイテク犯罪まで、推理世界の魅力的な謎を包括的に解読します。"
    ...
    const crimeMethodDescriptions = document.querySelectorAll(
    ...
    const crimeMethodTexts = {
      titles: [
        ...
        {
          zh: "偽裝自殺",
          en: "Disguised Suicide",
          ja: "偽装自殺",
          ko: "위장 자살",
        },
        {
          zh: "偽裝意外",
          en: "Disguised Accident",
          ja: "偽装事故",
          ko: "위장 사고",
        },
      ],
    ...
    crimeMethodDescriptions.forEach((desc, index) => {
      if (crimeMethodTexts.descriptions[index]) {
        desc.textContent =
          crimeMethodTexts.descriptions[index][currentLanguage];
      }
    });
    ...
    const caseCardDescriptions = document.querySelectorAll(
    ...
    const caseTexts = {
      sectionTitles: [
        ...
        {
          zh: "犯罪手法分類",
          en: "Crime Methods Classification",
          ja: "犯罪手法分類",
          ko: "범죄 수법 분류",
        },
        {
    ...
    sectionTitles.forEach((title) => {
      ...
      for (let i = 0; i < caseTexts.sectionTitles.length; i++) {
        if (
          titleText === caseTexts.sectionTitles[i].zh ||
    ...
    caseCardDescriptions.forEach((desc, index) => {
      if (caseTexts.caseCardDescriptions[index]) {
        desc.textContent =
          caseTexts.caseCardDescriptions[index][currentLanguage];
      }
    });
    ...
    statLabels.forEach((label) => {
      ...
      for (let i = 0; i < aboutTexts.statLabels.length; i++) {
        if (
          labelText === aboutTexts.statLabels[i].zh ||
  ...
  themeToggle.addEventListener("click", function () {
    ...
    currentTheme = document.body.classList.contains("light-theme")
      ? "light"
      : "dark";
  });
    ...
    fetch(`src/data/crime_methods.json`)
      ...
      .then((data) => {
        ...
        const filterBtns = document.querySelectorAll(
          ".crime-method-filters .filter-btn"
        );
      ...
      .catch((error) => {
        ...
        methodBody.innerHTML = `<p class="error-message">${
          currentLanguage === "zh"
            ? "無法載入犯罪手法內容"
            : currentLanguage === "en"
            ? "Failed to load crime method content"
            : currentLanguage === "ja"
            ? "犯罪手法の内容を読み込めませんでした"
  ...
  // 首頁案例卡片點擊
  const homePageCaseDetailBtns = document.querySelectorAll(
    ".case-card .case-detail-btn"
  );
