...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 創建一個計數器來追蹤數據加載狀態
    let dataLoadingCounter = 0;
    const totalDataSources = 3; // 總共有三個數據源需要加載
    
    // 定義一個函數來處理數據加載完成後的操作
    function handleDataLoaded() {
        dataLoadingCounter++;
        // 當所有數據都加載完成或嘗試加載後，隱藏載入畫面
        if (dataLoadingCounter >= totalDataSources) {
            hideLoadingScreen();
        }
    }
        ...
        if (chapterCards) {
            ...
            fetch(`src/data/chapters.json`)
                ...
                .catch((error) => {
                    ...
                    // 創建一個默認的章節數據，以便網站仍然可以顯示一些內容
                    chapters = [
                        {
                            id: "chapter1",
                            title: "前言",
                            description: "關於名偵探柯南系列和犯案手法的重要性"
                        },
                        {
                            id: "chapter2",
                            title: "基本概念",
                            description: "犯罪手法的分類與特點"
                        }
                    ];
    ...
    function loadChapterContent(chapterId) {
        ...
        fetch(`src/data/translations/${currentLanguage}/${chapterId}.md`)
            ...
            .catch(error => {
                ...
                contentBody.innerHTML = `
                    <div class="error-message">
                        <h2>${currentLanguage === 'zh' 
                            ? "內容暫時無法加載" 
                            : "Content temporarily unavailable"}</h2>
                        <p>${currentLanguage === 'zh'
                            ? "我們正在努力修復這個問題。請稍後再試。"
                            : "We are working to fix this issue. Please try again later."}</p>
                    </div>
                `;
            });
    }
    ...
    function loadCrimeMethods() {
        if (methodCards) {
            ...
            if (crimeMethods.length > 0 && methodCards.children.length > 0) {
                handleDataLoaded();
                return;
            }
            // 清空犯罪手法卡片容器
            ...
            fetch(`src/data/crime_methods.json`)
                ...
                .catch((error) => {
                    ...
                    // 創建一個默認的犯罪手法數據，以便網站仍然可以顯示一些內容
                    crimeMethods = [
                        {
                            id: "method1",
                            title: "密室謀殺",
                            category: "空間類",
                            description: "在密閉空間內實施的謀殺手法，通常涉及特殊的進出機制"
                        },
                        {
                            id: "method2",
                            title: "不在場證明偽造",
                            category: "時間類",
                            description: "通過各種手段製造虛假的不在場證明"
                        }
                    ];
    ...
    function loadCases() {
        if (caseCards) {
            // 如果已經載入過案例數據，則不重複載入
            if (cases.length > 0 && caseCards.children.length > 0) {
                handleDataLoaded();
                return;
            }
            // 清空案例卡片容器
            ...
            fetch(`src/data/cases.json`)
                ...
                .catch((error) => {
                    ...
                    // 創建一個默認的案例數據，以便網站仍然可以顯示一些內容
                    cases = [
                        {
                            id: "case1",
                            title: "鐘樓密室案",
                            method: "method1",
                            description: "在鐘樓內發生的密室謀殺案，兇手利用特殊機關製造密室"
                        },
                        {
                            id: "case2",
                            title: "雪山別墅案",
                            method: "method2",
                            description: "在雪山別墅中發生的謀殺案，兇手巧妙偽造不在場證明"
                        }
                    ];
                    ...
                    cases.forEach((caseItem) => {
                        ...
                        caseCard.innerHTML = `
                            <h3>${caseItem.title}</h3>
                            <p>${caseItem.description}</p>
                            <span class="method-tag">未知手法</span>
                        `;
                        caseCard.addEventListener("click", () => {
                            // 顯示一個提示，說明內容暫時無法加載
                            if (caseBody) {
